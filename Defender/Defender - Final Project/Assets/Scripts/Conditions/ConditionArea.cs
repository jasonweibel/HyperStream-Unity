...
public class ConditionArea : ConditionBase
{
    ...
    private void OnTriggerEnter2D(Collider2D otherCollider)
	{
	    ...
	    if (eventType != ColliderEventTypes.Enter) return;
	...
	//{
	//    //is this the type of event we need?
	//    if (eventType != ColliderEventTypes.StayInside || !(Time.time >= lastTimeTriggerStayCalled + frequency)) return;
	...
	//{
	//    //is this the type of event we need?
	//    if (eventType != ColliderEventTypes.Exit) return;
